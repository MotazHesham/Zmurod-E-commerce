            ...
            @csrf
                    ...
                    <div class="form-group col-6">
                        ...
                        <select class="form-control select2 {{ $errors->has('post_forum') ? 'is-invalid' : '' }}" name="post_forum_id" id="post_forum_id" required>
                            @foreach($post_forums as $id => $entry)
                                <option value="{{ $id }}" {{ old('post_forum_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-4">
                        ...
                        <select class="form-control select2 {{ $errors->has('post_tags') ? 'is-invalid' : '' }}" name="post_tags[]" id="post_tags" multiple>
                            @foreach($post_tags as $id => $post_tag)
                                <option value="{{ $id }}" {{ in_array($id, old('post_tags', [])) ? 'selected' : '' }}>{{ $post_tag }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group mx-auto p-2 d-flex justify-content-center">
                    <button class="btn btn-success col-3 " type="submit">
                        {{ trans('global.save') }}
                    </button>
                </div>
